...
from evolocity import logging as logg
from evolocity.preprocessing.neighbors import (
    compute_connectivities_umap,
    get_neighs,
    neighbors,
    verify_neighbors,
)
from evolocity.preprocessing.utils import sum_var
from evolocity.tools.utils import scale
from evolocity.tools.velocity_model import velocity_model
...
#     )
def likelihood_confidence(
        seq1,seq2,vocabulary,model,
        seq_cache={}, verbose=False, natural_aas=None):
    
    return seq_cache[seq2]-seq_cache[seq1]
...
class VelocityGraph:
    def __init__(
        ...
        seq_cache=gisaid_score.set_index('seq')["mean_score"].to_dict()
        attack_score=pd.read_csv('/home/chenn0a/chenn0a/covid_esm1b/AttackProtein2/analysis/data/attack_score.csv',index_col=0)
        temp=attack_score.set_index('seq')["mean_score"].to_dict()
        gisaid_attack_score=pd.read_csv('/home/chenn0a/chenn0a/covid_esm1b/AttackProtein2/analysis/data/gisaid_attack_score.csv',index_col=0)
        temp2=gisaid_attack_score.set_index('seq')["mean_score"].to_dict()
        seq_cache.update(temp)
        seq_cache.update(temp2)
        self.seq_probs = seq_cache
    ...
    def compute_likelihoods(self, vocabulary, model):
        gisaid_score=pd.read_csv('/home/chenn0a/chenn0a/covid_esm1b/AttackProtein2/analysis/data/gisaid_score.csv',index_col=0)
        self.self_prob=gisaid_score.set_index('seq')["mean_score"].to_dict()
    ...
    def compute_gradients(self, vocabulary, model):
        ...
        for i in iterator:
            ...
            if self.score == 'confidence':
                score_fn = likelihood_confidence
            # elif self.score == 'confidence':
            #     score_fn = likelihood_confidence
